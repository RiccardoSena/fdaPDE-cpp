    ...
    public:
     ...
     DVector<double> p_value(CIType type){
        ...
        if(type == simultaneous){  
            ...
            // Threshold used to determine how many components will not be flipped: we drop those that show large alpha_hat w.r.t. the expected standar error
            double threshold = 10*Sigma_hat; 
            int N_Eig_Out=0; // Store the number of biased components that will be kept fixed if enhanced-ESF is required
